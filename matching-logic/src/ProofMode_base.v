...
Section with_signature.
  ...
  Record named_hypothesis := mkNH
    {
      nh_name : string;
      nh_patt : Pattern;
    }.

  Definition hypotheses := list named_hypothesis.

  Definition names_of (h : hypotheses) : list string := map nh_name h.
  Definition patterns_of (h : hypotheses) : list Pattern := map nh_patt h.

  Definition has_name (n : string) (nh : named_hypothesis) : Prop
  := nh_name nh = n.

  #[global]
  Instance has_name_dec n nh : Decision (has_name n nh).
  Proof.
    unfold has_name.
    solve_decision.
  Defined.

  Definition find_hyp (name : string) (hyps : hypotheses) : option (nat * named_hypothesis)%type
  := stdpp.list.list_find (has_name name) hyps.


  Record MLGoal : Type := mkMLGoal
    { mlTheory : Theory;
  ...
  Definition MLGoal_from_goal
    (Γ : Theory) (goal : Pattern) (pi : ProofInfo)
    :
    MLGoal
    := @mkMLGoal Γ nil goal pi.

  Coercion of_MLGoal (MG : MLGoal) : Type :=
...
Section with_signature.
  Import ProofMode_base.Notations.
  Context {Σ : Signature}.
  Open Scope ml_scope.
  ...
  Ltac useBasicReasoning :=
    unfold ProofSystem.derives;
    lazymatch goal with
    | [ |- of_MLGoal (@mkMLGoal _ _ _ _) ] => apply mlUseBasicReasoning
  ...
  Local Example ex_extractWfAssumptions Γ (p : Pattern) :
    well_formed p ->
  ...
  Lemma cast_proof' (Γ : Theory) (ϕ ψ : Pattern) (i : ProofInfo) (e : ψ = ϕ) :
    Γ ⊢i ϕ using i ->
    Γ ⊢i ψ using i.
  Proof.
    intros [pf Hpf].
...
End with_signature.
  ...
  Lemma cast_proof_ml_hyps Γ hyps hyps' (e : patterns_of hyps = patterns_of hyps') goal (i : ProofInfo) :
    @mkMLGoal Γ hyps goal i ->
    @mkMLGoal Γ hyps' goal i.
  Proof.
    ...
    unshelve (eapply (@cast_proof' Γ _ _ i _ H)).
    rewrite e.
    reflexivity.
  Defined.

  Lemma cast_proof_ml_goal Γ hyps goal goal' (e : goal = goal') (i : ProofInfo):
    @mkMLGoal Γ hyps goal i ->
    @mkMLGoal Γ hyps goal' i .
  Proof.
    unfold of_MLGoal. simpl. intros H.
  ...
  Lemma MLGoal_intro (Γ : Theory) (l : hypotheses) (name : string) (x g : Pattern)
    (i : ProofInfo) :
    @mkMLGoal Γ (l ++ [mkNH name x]) g i ->
    @mkMLGoal Γ l (x ---> g) i.
  Proof.
    intros H.
  ...
  Ltac simplLocalContext :=
    match goal with
      | [ |- @of_MLGoal (@mkMLGoal ?Ctx ?l ?g ?i) ]
        => eapply cast_proof_ml_hyps;[(rewrite {1}[l]/app; reflexivity)|]
    end.

  Ltac _getHypNames :=
    lazymatch goal with
    | [ |- of_MLGoal (@mkMLGoal _ ?l _ _) ] => eval lazy in (names_of l)
    end.

  Tactic Notation "_failIfUsed" constr(name) :=
    lazymatch goal with
    | [ |- of_MLGoal (@mkMLGoal _ ?l _ _) ] =>
  ...
  Local Example ex_toMLGoal Γ (p : Pattern) :
    well_formed p ->
    Γ ⊢i p ---> p using BasicReasoning.
  Proof.
    intros wfp.
    ...
    { wf_auto2. }
    match goal with
    | [ |- of_MLGoal (@mkMLGoal Γ [] (p ---> p) BasicReasoning) ] => idtac
    | _ => fail
    end.
    fromMLGoal.
  Abort.

  Local Example ex_mlIntro Γ a (i : ProofInfo) :
    well_formed a ->
  ...
  Lemma MLGoal_revertLast (Γ : Theory) (l : hypotheses) (x g : Pattern) (n : string) i :
