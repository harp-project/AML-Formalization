...
Section with_signature.
  ...
  Arguments P1 _ (_%ml) (_%ml) _ _ .
  Arguments P2 _ (_%ml) (_%ml) (_%ml) _ _ _.
  Arguments P3 _ (_%ml) _.
  ...
  Lemma useGenericReasoning'  (Γ : Theory) (ϕ : Pattern) i' i:
  (ProofInfoLe i' i) ->
  Γ ⊢i ϕ using i' ->
  Γ ⊢i ϕ using i.
  Proof.
  ...
  Lemma mlUseGenericReasoning
    (Γ : Theory) (l : hypotheses) (g : Pattern) (i i' : ProofInfo) :
    ProofInfoLe i i' ->
    @mkMLGoal Σ Γ l g i ->
    @mkMLGoal Σ Γ l g i'.
  Proof.
  ...
  Lemma MLGoal_exact Γ l name g idx info:
  ...
  Local Example ex_mlExact Γ a b c:
    well_formed a = true ->
    well_formed b = true ->
    well_formed c = true ->
  ...
  Lemma MLGoal_weakenConclusion_under_first_implication Γ l name g g' i:
  ...
  Lemma MLGoal_weakenConclusion Γ l₁ l₂ name g g' i:
  ...
  Local Example ex_mlApplyn Γ a b:
    well_formed a ->
    well_formed b ->
  ...
  Lemma mlGoal_assert Γ l name g h i:
  well_formed h ->
  @mkMLGoal Σ Γ l h i ->
  ...
  Lemma mlGoal_assert_generalized Γ l1 l2 name g h i:
  well_formed h ->
  @mkMLGoal Σ Γ l1 h i ->
  ...
  Local Example ex_assert_using Γ p q a b:
    well_formed a = true ->
    well_formed b = true ->
    well_formed p = true ->
    well_formed q = true ->
  ...
  Lemma MLGoal_add Γ l name g h i:
  Γ ⊢i h using i ->
  ...
  Tactic Notation "mlAdd" constr(n) :=
    ...
    Lemma mlGoal_clear_hyp Γ l1 l2 g h i:
      @mkMLGoal Σ Γ (l1 ++ l2) g i ->
  ...
  Local Example ex_mlClear Γ a b c:
  well_formed a ->
  well_formed b ->
  well_formed c ->
  Γ ⊢i a ---> (b ---> (c ---> b)) using BasicReasoning.
  ...
  *)
  ...
  Lemma MLGoal_disj_elim Γ l₁ l₂ pn p qn q pqn r i:
  ...
  Lemma MLGoal_applyMetaIn Γ n r n' r' i:
    Γ ⊢i (r ---> r') using i ->
    forall l₁ l₂ g,
  ...
  Local Example Private_ex_mlApplyMetaRawIn Γ p q:
    well_formed p ->
    well_formed q ->
    Γ ⊢i p ---> (p or q)
    using BasicReasoning.
  ...
  *)
  ...
  Lemma MLGoal_destructAnd Γ g l₁ l₂ nx x ny y nxy i:
  ...
  Local Example ex_mlDestructAnd Γ a b p q:
    well_formed a ->
    well_formed b ->
    well_formed p ->
    well_formed q ->
  ...
  Lemma prf_conj_split_meta Γ a b l (i : ProofInfo):
    well_formed a ->
    well_formed b ->
    Pattern.wf l ->
  ...
  Lemma prf_conj_split_meta_meta Γ a b l (i : ProofInfo):
    well_formed a ->
    well_formed b ->
    Pattern.wf l ->
    Γ ⊢i (foldr patt_imp a l) using i -> 
  ...
  Lemma MLGoal_splitAnd Γ a b l i:
    @mkMLGoal Σ Γ l a i ->
    @mkMLGoal Σ Γ l b i ->
  ...
  Local Lemma ex_mlSplitAnd Γ a b c:
    well_formed a ->
    well_formed b ->
    well_formed c ->
    Γ ⊢i a ---> b ---> c ---> (a and b)
  ...
  Lemma prf_local_goals_equiv_impl_full_equiv Γ g₁ g₂ l:
    well_formed g₁ ->
    well_formed g₂ ->
    Pattern.wf l ->
  ...
  Lemma prf_local_goals_equiv_impl_full_equiv_meta Γ g₁ g₂ l i:
    well_formed g₁ ->
    well_formed g₂ ->
    Pattern.wf l ->
  ...
  Lemma prf_local_goals_equiv_impl_full_equiv_meta_proj1 Γ g₁ g₂ l i:
    well_formed g₁ ->
    well_formed g₂ ->
    Pattern.wf l ->
    Γ ⊢i (foldr patt_imp (g₁ <---> g₂) l) using i ->
  ...
  Lemma prf_local_goals_equiv_impl_full_equiv_meta_proj2 Γ g₁ g₂ l i:
    well_formed g₁ ->
    well_formed g₂ ->
    Pattern.wf l ->
    Γ ⊢i (foldr patt_imp (g₁ <---> g₂) l) using i ->
  ...
  Lemma not_phi_iff_phi_bott Γ ϕ :
    well_formed ϕ ->
    Γ ⊢i (! ϕ ) <---> (ϕ <---> ⊥)
    using BasicReasoning.
  Proof.
  ...
  Lemma not_not_iff (Γ : Theory) (A : Pattern) :
    well_formed A ->
    Γ ⊢i A <---> ! ! A
    using BasicReasoning.
  Proof.
  ...
  Local Lemma well_formed_foldr_and (x : Pattern) (xs : list Pattern):
    well_formed x ->
    Pattern.wf xs ->
  ...
  Lemma lhs_and_to_imp Γ (g x : Pattern) (xs : list Pattern):
    well_formed g ->
    well_formed x ->
    Pattern.wf xs ->
  ...
  Lemma lhs_and_to_imp_meta Γ (g x : Pattern) (xs : list Pattern) i:
    well_formed g ->
    well_formed x ->
    Pattern.wf xs ->
  ...
  Lemma and_singleton : forall Γ p,
   ...
   *)
   Example ex_and_of_equiv_is_equiv_2 Γ p q p' q' i:
   well_formed p ->
   well_formed q ->
   well_formed p' ->
   well_formed q' ->
  ...
  #[local]
    Example ex_or_of_equiv_is_equiv_2 Γ p q p' q' i:
      well_formed p ->
      well_formed q ->
      well_formed p' ->
      well_formed q' ->
